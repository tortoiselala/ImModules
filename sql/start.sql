...
create table imm.user  (
    ...
    username varchar(30) character set utf8 not null comment '用户名(nick_name)',
    password varchar(50) character set utf8 not null comment '密码(MD5(密码+盐))',
    salt varchar(20) character set utf8 null default null comment '盐',
    real_name varchar(30) character set utf8 null default null comment '用户真名',
    phone varchar(20) character set utf8 null default null comment '电话号码(唯一)',
    email varchar(50) character set utf8 null default null comment '邮件地址(唯一)',
    sex tinyint(4) not null comment '性别(1.男 2.女)',
    status tinyint(4) default null comment '账户状态(1.正常 2.锁定 3.删除 4.非法)',
    create_time bigint default unix_timestamp(now()) comment '创建时间',
    update_time bigint default unix_timestamp(now()) comment '更新时间',
...
create table imm.account_log  (
    id bigint  not null auto_increment comment '用户账户操作日志主键',
    ...
    create_time bigint null default unix_timestamp(now()) comment '创建时间',
...
create table imm.operation_log  (
    id bigint not null auto_increment comment '用户操作日志主键',
